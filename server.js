...
app.post('/register-warranty', upload.single('billPdf'), async (req, res) => {
    ...
    try {
        ...
        else
        {
            ...
            res.send('<script>alert("Warrenty Registration Sucessfully."); window.history.back();</script>');
        }
...
app.post('/verify-warranty', async (req, res) => {
    const { serialNumber, purchaseDate, email, duration } = req.body;
...
const formattedDate2 = expiryDateObj.toLocaleDateString(undefined, options);
        ...
        if (warranty) {
            ...
            res.send('<script>alert("Warrenty Verified Sucessfully."); window.history.back();</script>');
        } else {
...
app.post('/admin/login', (req, res) => {
    ...
    const credentials = {
        SuperAdmin: { username: 'Admin', password: 'Bitbox@112233' },
        DriverAdmin: { username: 'DriverAdmin', password: 'DriverAdmin' },
        WarrantyAdmin: { username: 'WarrantyAdmin', password: 'WarrantyAdmin' },
        InventoryAdmin: { username: 'InventoryAdmin', password: 'InventoryAdmin' }
    };
